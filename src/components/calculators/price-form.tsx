    ...
    <Form {...form}>
      <form className="space-y-8">
        <div className="space-y-4">
          <div className={cn("hidden md:grid gap-4 px-6 py-3 text-[10px] font-black uppercase tracking-[0.3em] text-primary/60 bg-white/5 rounded-xl border border-white/5", gridLayout)}>
            <span>Length</span>
            <span>Width</span>
            <span>Substance</span>
          ...
          <div className="space-y-4 md:space-y-3">
            {fields.map((field, index) => {
              ...
              return (
                <div key={field.id} className={cn("relative grid gap-3 md:gap-4 items-center glass-panel p-5 md:px-6 md:py-2 rounded-2xl group transition-all duration-300 hover:bg-white/5", gridLayout)}>
                  ...
                  <FormField control={form.control} name={`rows.${index}.panjang`} render={({ field }) => (
                    <FormItem className="space-y-1">
                      ...
                      <FormControl><Input {...field} type="number" className="h-10 bloomberg-input font-mono" /></FormControl>
                    </FormItem>
                  )} />
                  ...
                  <FormField control={form.control} name={`rows.${index}.substance`} render={({ field }) => (
                    <FormItem className="col-span-2 md:col-span-1 space-y-1">
                      ...
                      <FormControl><Input {...field} className="h-10 bloomberg-input font-mono text-xs" /></FormControl>
                    </FormItem>
                  )} />
                  ...
                  <FormField control={form.control} name={`rows.${index}.flute`} render={({ field }) => (
                    <FormItem className="space-y-1">
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        <FormControl><SelectTrigger className="h-10 bloomberg-input font-bold"><SelectValue /></SelectTrigger></FormControl>
                  ...
                  <FormField control={form.control} name={`rows.${index}.diskon`} render={({ field }) => (
                    <FormItem className="space-y-1">
                      ...
                      <FormControl><Input {...field} type="number" className="h-10 bloomberg-input font-mono text-center" /></FormControl>
                    </FormItem>
                  )} />
                  ...
                  <div className="col-span-2 md:col-span-1 h-auto md:h-10 flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center font-mono bg-primary/5 md:bg-transparent p-3 md:p-0 rounded-xl">
                    ...
                    <div className="flex flex-col items-end">
                      {isPriceNotFound ? (
                          ...
                          <>
                              <span className="text-base md:text-lg font-black text-primary drop-shadow-[0_0_8px_rgba(255,184,0,0.3)]">{currencyFormatter.format(rowPrice ?? 0)}</span>
                              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter opacity-60">
                                {isFinite(rowMOQ) ? `MOQ ${rowMOQ.toLocaleString()}U` : 'N/A'}
                              </span>
                          </>
                      )}
                    </div>
                  ...
                  <div className="hidden md:flex items-center justify-end">
                    {fields.length > 1 && (
                      <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all rounded-xl" onClick={() => remove(index)}>
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    )}
                  </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
            <Button type="button" variant="outline" className="h-12 w-full sm:w-auto glass-panel border-dashed hover:border-primary hover:bg-primary/5 transition-all group" onClick={() => append({ panjang: 0, lebar: 0, substance: "M100/M100/M100", flute: "B", diskon: 0 })}>
                <PlusCircle className="mr-2 h-5 w-5 group-hover:rotate-90 transition-transform" /> Add Terminal Row
            </Button>
            <Button type="button" variant="secondary" className="h-12 w-full sm:w-auto font-black uppercase tracking-widest bg-white/10 hover:bg-white/20 transition-all shadow-xl" onClick={() => setIsSimulating(!isSimulating)}>
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    <Card className="lg:col-span-2 glass-panel p-0 overflow-hidden">
                        <CardHeader className="bg-white/5 border-b border-white/5 p-6">
                            ...
                            <CardDescription className="uppercase text-[10px] font-black tracking-[0.3em] text-muted-foreground">Input order volume per sequence</CardDescription>
                        </CardHeader>
                                    ...
                                    <div key={index} className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${index * 100}ms` }}>
                                        <div className="flex flex-col sm:flex-row justify-between items-start gap-6">
                                            <div className="space-y-1">
                                                <p className="text-lg font-black tracking-tight flex items-center gap-2">
                                                    ...
                                                    <span className="text-xs text-muted-foreground font-mono bg-white/5 px-2 py-0.5 rounded border border-white/5">{row.substance} ({row.flute})</span>
                                                </p>
                                                <div className="flex items-center gap-3 text-[11px] font-bold text-primary/40 uppercase tracking-widest">
                                                    <Weight className="h-3 w-3" />
                                                    <span>NET: {weightPerPcsInKg.toFixed(4)} KG/U</span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                                            <div>
                                                ...
                                                <p className="font-mono text-foreground font-black text-base">{itemTotalWeight.toFixed(3)} <span className="text-[10px] opacity-40">TONS</span></p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-[10px] text-muted-foreground uppercase font-black tracking-[0.2em] mb-1">Subtotal</p>
                                                <p className="font-mono text-primary font-black text-base drop-shadow-[0_0_10px_rgba(255,184,0,0.2)]">{currencyFormatter.format(itemTotal)}</p>
                                            </div>
                                        </div>
